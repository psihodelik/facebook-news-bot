...
exports.Events = {
  ...
  support(user) {
    const front = user.front === "international" ? "uk" : user.front
    const cmp = "?" + CAMPAIGN_CODE_PARAM + "_support"
    Facebook.sendMessage(
      user.ID,
      buildButtonsAttachment(
        "Producing well-reported journalism is difficult and expensive. Supporting us isn't. Please support the Guardian.",
        [
          buildLinkButton("https://contribute.theguardian.com/"+ front + cmp, "Contribute"),
          buildLinkButton("https://membership.theguardian.com/"+ front +"/supporter" + cmp, "Become a Supporter")
        ]
      )
    )
  }
    ...
    att.quick_replies =
      [buildQuickReply(
        ...
        .concat(buildTopicQuickReplies(type, user.front, topic),
                [buildQuickReply("Support the Guardian", buildPayload("support"))])
...
function buildLinkButton(url, title) {
  return {
    "type": "web_url",
    "url": url,
    "title": title
